...
AdapterAscoltatore.prototype.add = function(id, room, fn){
  if(!this.funcs[id]) {
    this.rooms[id] = [];
    var that = this;
    this.funcs[id] = function(topic, data) {
      if(data.except.indexOf(id) === -1) {
        that.nsp.connected[id].packet(data.packet);
      }
    }
    this.ascoltatore.subscribe("broadcast", this.funcs[id]);
  };
  this.rooms[id].push(room);
  this.ascoltatore.subscribe(room, this.funcs[id], fn);
};
  ...
  async.series([
    ...
    function(cb) {
      ...
      if(that.rooms[id].length === 0) {
        that.ascoltatore.unsubscribe("broadcast", that.funcs[id]);
